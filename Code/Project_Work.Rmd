...
#8 Add linear interpolation to missing data Live Count
salmon_data_interpolated <- 
  salmon_data %>% 
  mutate(LiveTotal = zoo::na.approx(LiveTotal, xout = SurveyDate)) %>% 
  mutate(NewReddCount = zoo::na.approx(NewReddCount, xout = SurveyDate))#Learned xout from chatGPT, since we were getting an error message about a size mismatch when interpolating
...
#10 Generate time series for monthly average live counts 
...
salmonredd.monthly.ts <- ts(salmon_data1$MeanRedd,
                   start=c(f_year,f_month),
                   frequency=12)
...
# Inspect results -- fail to reject H0 
redd_data_trend
